<?php
class FilemultiTest extends PHPUnit_Framework_TestCase
{
    ...
    *******************************************************************************/
    ...
    {
        $file = new \Upload\File('bar', new \Upload\Storage\FileSystem($this->assetsDirectory), 0);
    }
    ...
    public function testValidationFailsIfNotUploadedFile()
    {
        $file = $this->getMock(
            '\Upload\File',
            array('isUploadedFile'),
            array('foo', new \Upload\Storage\FileSystem($this->assetsDirectory), 0)
        );
        $file->expects($this->any())
             ->method('isUploadedFile')
